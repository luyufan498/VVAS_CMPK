...
void updatePerformanceInfo(ivas_xoverlaypriv *kpriv)
{
  struct performance_info *perinfo = &(kpriv->perinfo);

  
  perinfo->frames_displayed_time.push_back(get_time ());

  auto recordlen = (perinfo->frames_displayed_time.end() - perinfo->frames_displayed_time.begin())/1000;

  if(recordlen > perinfo->cal_time_ms)
  {
    int frame_num = perinfo->frames_displayed_time.size();

    perinfo->avgFPS = (float)frame_num /  ((float)recordlen/1000);
...
int pushbackPoint(ivas_xoverlaypriv *kpriv){
  ...
  LOG_MESSAGE (LOG_LEVEL_DEBUG,kpriv->log_level, "SOM total power:%ld, Converted y:%d, read_cnt: %d",pp.data,pp.y_converted,readcnt++);
  //读取power数据
...
int drawMaxMinPowerInfo(ivas_xoverlaypriv *kpriv){
  ...
  drawOverylayStrLineYUV(kpriv,title[kpriv->sampleinfo.targetid],0);
  // LOG_MESSAGE (LOG_LEVEL_DEBUG, kpriv->log_level, "p info title2");
  char buff[60]={0};
  sprintf(buff,"Max:%3.2f Min:%3.2f Avg:%3.2f",(float)datamax/1000,(float)datamin/1000,(float)datasum/len/1000);
